...
Goldreich, micali y Wigdergon demostraron en [TODO] que el problema G3C tiene una prueba de conocimiento cero computacional. Para la prueba y su demostración se debe introducir la teoría cifrado probabilístico que permite ocultar a V la 3-coloración que P ha encontrado en el primer mensaje, el testigo. La idea es que el cifrado de todo el grafo coloreado no se puede deshacer sin conocer la clave, la cual se entrega como \textit{respuesta} para un solo vértice coloreado que elige aleatoriamente V en el \textit{reto}.

Además se sabe que G3C pertenece a \textbf{NPC} (\ref{redNPC:theo}), por lo que se deduce que todo problema en \textbf{NPC} tiene una ZKP computacional.
