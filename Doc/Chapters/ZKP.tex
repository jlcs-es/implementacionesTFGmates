...
\rule{\textwidth}{1pt}
\begin{algorithm}[Compromiso de bit basado en residuos cuadráticos]
	\hfil
...
A continuación, presentamos otro esquema de compromiso basado en el logaritmo discreto. Para llevarlo a cabo, P y V deben primero decidir un $p$ primo suficientemente grande y un generador $g\in \mathbb{Z}_p^*$ donde el problema del logaritmo discreto es \textit{difícil}. V además elige un valor $s\in \mathbb{Z}_{p-1}$ y lo envía a P. Los conjuntos del esquema de compromiso son $\Upsilon = \{0,1,\dots,p-2\}$ para las \textit{claves} y $\chi= \mathbb{Z}_p^*$ para los blobs.
...
El posible ataque a este esquema de compromiso consistiría en calcular el logaritmo discreto de $s$ módulo $p$, llamémoslo $t$, $g^=s\,mod\,p$. Si P oculta $b=0$, enviará $x=g^y\,mod\,p$, y para poder abrir al valor $b=1$ puede enviar a V $y-t$, de modo que V calculará $sg^{y-t}\equiv g^t g^{y-t} \equiv g^y \equiv x \, mod \, p$, obteniendo $b=1$. En el caso contrario, donde P oculta $b=1$, enviará $x=sg^y\,mod\, p$ a V como testigo, y para abrir el bit como $b=0$ basta con enviar $y+t$, tal que V calculará $g^{y+t}\equiv g^t g^y \equiv s g^y\, mod\, p$, creyendo entonces que $b=1$.

Como antes, si P es computacionalmente limitado, no podrá resolver el problema del logaritmo discreto y obtenemos una \textbf{vinculación computacional}. También tenemos como antes que los ensambles $\{f(0,\Upsilon)\}$ y $\{f(1,\Upsilon)\}$ son iguales, y por tanto el esquema es de \textbf{secreto incondicional}.
...
En este apartado presentaremos dos esquemas de compromiso que consiguen \textit{vinculación incondicional}, al contrario que los casos anteriores. El primero utilizará, de nuevo, los residuos cuadráticos, y el segundo el problema del logaritmo discreto.
...
% TODO: DL unconditional binding


% TODO : mulivalued blobs, commit strings

% TODO  : párrafo/s de cierre relacionando con ZKP