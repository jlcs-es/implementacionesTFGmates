...
\begin{frame}{Primality Test}
	\begin{description}[Parameters]
		\item[Name] Primality problem (PRIM).
		\item[Parameters] Positive integer $N$.
		\item[Question] Is $N$ prime?
	\end{description}
\end{frame}
...
\begin{frame}{Pruebas de Conocimiento Cero: Problema QR}
	...
	\begin{itemize}
		...
		\item %Por construcción, dados $w$ y $b$ en el simulador, $u$ tiene un único valor posible, $u\equiv w^2x^{-b}\,mod\,N$, por tanto, la probabilidad de que dada la tupla $(u, b, w)$, $U_i$ tenga el valor $u$ condicionado a que $ B_i=b$ y que $W_i=w$, es 1:
...
\begin{frame}{Otros tipos de Pruebas de Conocimiento Cero}
	\begin{description}[Verificador Honesto]
		...
		\item[]
		\item[Estadísticas] Igualdad asintótica de los ensambles: diferencia estadística es una función \textit{despreciable}.
